...
export const transporter = nodemailer.createTransport({
    ...
    port: Number(process.env.BREVO_STMP_PORT),
    secure: false,
    auth: {
...
transporter.verify((err, success)=> {
    if(err) console.log(`Erreur SMTP`, err.message);
    else console.log(`SMTP connecté`);
    
    
});

export const sendVeerificationMail = async (email, token) => {
    await transporter.sendMail({
        from: `Authentification API <lucievyns@gmail.com`,
        to: email,
